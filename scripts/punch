...
default_commit_message = "Version update {{ current_version }} -> {{ new_version }}"
...
if args.set_part and args.reset_on_set:
    ...
    if len(set_parts) > 1:
        fatal_error("If you specify --reset-on-set you may set only one value")
...
current_version_string, new_version_string = global_replacer.run_main_serializer(current_version.as_dict(),
                                                                                 new_version.as_dict())
if config.vcs is not None:
    ...
    vcs_configuration = vcsc.VCSConfiguration.from_dict(config.vcs, config.globals, special_variables)
else:
...
if args.simulate:
    ...
    changes = global_replacer.run_all_serializers(current_version.as_dict(), new_version.as_dict())
    ...
    if vcs_configuration is not None:
        if vcs_configuration.name == 'git':
            ...
            fatal_error("The requested version control system {} is not supported.".format(vcs_configuration.name))

        try:
            repo = repo_class(os.getcwd(), vcs_configuration)
        except rex.RepositorySystemError as exc:
            fatal_error("An error occurred while initializing the version control repository", exc)
    else:
        repo = None

    if vcs_configuration is not None:
        # TODO: Create a fake UseCase to allow running this without a repo and outside this nasty if
