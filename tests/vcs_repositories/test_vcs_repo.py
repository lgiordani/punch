...
def test_init_without_program_installed(temp_empty_uninitialized_dir):
    with mock.patch("subprocess.Popen") as mock_popen:
        ...
        with pytest.raises(re.RepositorySystemError):
            ...
            vr.VCSRepo(temp_empty_uninitialized_dir)


def test_run_without_errors(temp_empty_uninitialized_dir):
    with mock.patch('subprocess.check_call'):
        vr.VCSRepo._set_command = _test_set_command
        repo = vr.VCSRepo(temp_empty_uninitialized_dir)
...
def test_run_with_errors(temp_empty_uninitialized_dir):
    with mock.patch('subprocess.check_call'):
        vr.VCSRepo._set_command = _test_set_command
        repo = vr.VCSRepo(temp_empty_uninitialized_dir)
